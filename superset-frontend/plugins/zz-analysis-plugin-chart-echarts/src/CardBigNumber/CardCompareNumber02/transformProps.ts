...
export default function transformProps(
  ...
  let sevenDayAvg: number | null = null;
  if (showSevenDayAvg && data.length >= 7) {
    const last7Days = data.slice(-7);
    const sum = last7Days.reduce(
      (acc, row) => acc + (parseMetricValue(row[metricName]) || 0),
      0,
    );
    sevenDayAvg = sum / 7;
  } else if (showSevenDayAvg && data.length > 0 && data.length < 7) {
    // If less than 7 days, calculate average of available data
    const sum = data.reduce(
      (acc, row) => acc + (parseMetricValue(row[metricName]) || 0),
      0,
    );
    sevenDayAvg = sum / data.length;
  }
  ...
  return {
    ...
    metricNameFontSize,
    title: title || '',
    titleFontSize: titleFontSize ?? 0.2,
    sevenDayAvg,
    showSevenDayAvg,
    numberFormatter,
  };
