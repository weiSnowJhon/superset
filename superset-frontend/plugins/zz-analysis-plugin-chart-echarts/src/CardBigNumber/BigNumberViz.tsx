...
function BigNumberVis({
  ...
  const rendermetricComparisonSummary = (maxHeight: number) => {
    const { width, percentChange, compareSuffix } = props;
    let fontSize = 0;
    ...
    if (text) {
      ...
      // 正数时显示红色，负数显示绿色
      const valueColor = percentChange !== undefined ? (percentChange > 0 ? '#ff4d4f' : (percentChange < 0 ? '#52c41a' : undefined)) : undefined;

      // 将文本拆分为前缀和数值部分
      const parts = text.split(' ');
      const prefix = compareSuffix || '';
      const value = parts.length > 1 ? parts.slice(1).join(' ') : text;
  ...
  const renderAvgValueSummary = (maxHeight: number) => {
    const { avgValue, avgSubheader, width } = props;
    
    if (avgValue === undefined || avgValue === null || !avgSubheader) {
      return null;
    }

    const text = `${avgSubheader} ${headerFormatter(avgValue)}`;
    ...
    return (
      <div
        className="avg-line subheader-line"
        style={{
          fontSize,
          height: maxHeight,
        }}
  ...
  const renderComparisonAndAverage = (maxHeight: number) => {
    const { width, percentChange } = props;
    const comparisonContent = rendermetricComparisonSummary(maxHeight);
    const avgContent = renderAvgValueSummary(maxHeight);

    // 如果两个都没有内容，返回 null
    if (!comparisonContent && !avgContent) {
      return null;
    }

    // 如果只有一个有内容，返回该内容
    if (!comparisonContent) return avgContent;
    if (!avgContent) return comparisonContent;

    // 两个都有内容时，使用 flex 布局
    return (
      <div
        className="comparison-avg-container"
        style={{
          display: 'flex',
          width: '100%',
          gap: '8px',
  ...
  return (
    ...
    >
      <div className="text-container">
        ...
        {renderComparisonAndAverage(Math.ceil(subheaderFontSize * height))}
