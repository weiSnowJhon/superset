...
export default function transformProps(
  ...
  const avgLag = Number(avgLag_) || 0;
  let formattedSubheader = subheader;
  let avgValue: number | null = null;
  let formattedAvgSubheader = '';
    ...
    if (compareIndex < sortedData.length) {
      ...
      if (compareToValue !== null && compareFromValue !== null) {
        percentChange = compareFromValue
          ...
          : 0;
        formattedSubheader = `${compareSuffix} ${formatPercentChange(
          percentChange,
        )}`;
      }
    }
  ...
  // Calculate average value based on avgLag
  if (avgLag > 0 && sortedData.length > 0) {
    const avgCount = Math.min(avgLag, sortedData.length);
    let sum = 0;
    let count = 0;
    for (let i = 0; i < avgCount; i++) {
      const value = sortedData[i][1];
      if (value !== null) {
        sum += Number(value);
        count++;
      }
    ...
    if (count > 0) {
      avgValue = sum / count;
      formattedAvgSubheader = `${avgSuffix}`;
    }
  }
  ...
  return {
    ...
    refs,
    avgValue,
    avgSubheader: formattedAvgSubheader,
    percentChange,
    compareSuffix,
  };
}
