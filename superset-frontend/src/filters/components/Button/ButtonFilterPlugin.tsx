      ...
      >
        ...
        >
          ...
          <Space wrap size={[8, 8]} className="select-container">
            {options.map(opt => {
              // 判断当前按钮是否被选中
              const isSelected = ensureIsArray(filterState.value).includes(opt.value);
              ...
              return (
                <Button
                  ...
                  style={{
                    ...
                    borderRadius: '4px',
                    marginBottom: '4px', // 纵向微调
                  }}
                  size="small" // 这里可以改为你想要的变量
                  onClick={() => {
                    if (multiSelect) {
                      // 多选逻辑：已选则移除，未选则添加
                      ...
                      // 单选逻辑：点击已选的则清空，否则选中
                      handleChange(isSelected ? [] : [opt.value]);
                    }
                  }}
                  disabled={isDisabled}
                >
            ...
            {/* 如果需要保留“清除”功能，可以加一个重置按钮 */}
            {(filterState.value?.length > 0) && (
              <Button type="link" size="small" onClick={() => handleChange([])}>
                {t('Clear')}
              </Button>
            )}
          </Space>
